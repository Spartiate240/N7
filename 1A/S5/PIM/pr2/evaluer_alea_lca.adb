...
procedure Evaluer_Alea_LCA is
	...
	begin
		...
		for i in 1..Taille loop
			...
			begin
			Enregistrer(sda, nbr, La_Valeur(sda,nbr) +1);  --ajout 1 à la valeur de nbr, donc sa fréquence

			exception 
				when Cle_Absente_Exception => Enregistrer(sda,nbr,1);  --s'il n'y a pas la clé, on initialise
			end;
		end loop;


			--recherche des fréquences min et max, on le fait dans une boucle séparée pour une question de complexité
			--temporelle

		for i in 1..Borne loop
			begin
				if La_Valeur(sda,i) > Max
					then Max := La_Valeur(sda,i);
				end if;
				if La_Valeur(sda,i)< Min
					then Min := La_Valeur(sda,i);
				end if;
