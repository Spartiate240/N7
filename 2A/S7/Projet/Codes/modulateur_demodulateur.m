...
retard = length(h); % la moitié de l'ordre du filtre (ici *2 car symétrie)
% Initialisation TEBs
TEB = zeros(1,n);
TEB_s = zeros(1,n);
TEB_h = zeros(1,n);
courant = 1;
epsilon = 1*10^-1; % écart type
...
while courant<n
    ...
    %% Calcul TEB
    % On rÃ©duit le vecteur du nb de bits pour qu'il ait la mÃªme taille
    bits_TEB = bits(1: length(sig_re));
    TEB(courant) = sum(bits_TEB ~= sig_re)/length(bits_TEB);

    Nerreur = sum(bits_TEB ~= sig_re);
    if Nerreur>precision
        courant = courant+1;
    end
