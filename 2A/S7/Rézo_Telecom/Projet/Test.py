...
#liste_adj = liste_adja(liens_rezo)
...
# Entrée: appellant, appele: source et destination de l'appel
#           capacites_stat: matrice des capacités
# Sortie: chemin prédéfini ou liste vide si aucun chemin n'est défini
def routage_statique(appellant, appele, capacites_stat):
    chemin = []
    # Chemins prédéfinis
    ...
    # Vérification des capacités, on ne peut pas passer si la capacité est nule
...
#         capacites_par : matrice des capacités
# Sortie: chemin choisi ou liste vide si aucun chemin possible

def partage_charge(liste_adj, appellant, appele, capacites_par):
...
def routage_adaptatif(liste_adj, appellant, appele, capacites_adapt):
    chemins_p = trouver_chemins(liste_adj, appellant, appele)
...
# Sortie: appels: liste des appels générés de la forme [source, destination, duree]
...
# Entrées:
#           nb_appels: nombre d'appels par heure
#           liste_adj : liste d

def simulation(nb_appels, matrice_liens, capas_stat, capas_par, capas_dyn):
    rejets_stat, rejets_par_charge, rejets_dyn = 0, 0, 0
    appels_stat, appels_par_charge, appels_dyn = [], [], []

    # Comme ça le changement de la matrice des liens change directement ici
    liste_adj = liste_adja(matrice_liens)
    ...
    for min in range(30):
        ...
        for appel in generee:
            ...
            chemin_stat = routage_statique(source, destination, capas_stat)
            temps_stat += time.time() - start_time  # Temps écoulé

            if chemin_stat:
                maj_capacites(chemin_stat, capas_stat, -1)
            ...
            chemin_par_charge = partage_charge(liste_adj, source, destination, capas_par)
            temps_par_charge += time.time() - start_time

            if chemin_par_charge:
                maj_capacites(chemin_par_charge, capas_par, -1)
            ...
            start_time = time.time()
            chemin_dyn = routage_adaptatif(liste_adj, source, destination, capas_dyn)
            temps_dyn += time.time() - start_time

            if chemin_dyn:
                maj_capacites(chemin_dyn, capas_dyn, -1)
        ...
        for i in range(len(appels_stat) - 1, -1, -1):
            ...
            duree -= 1
            if duree <= 0:
                maj_capacites(chemin, capas_stat, 1)
                appels_stat.pop(i)
            else:
        ...
        for i in range(len(appels_par_charge) - 1, -1, -1):  # Parcourir à l'envers pour éviter les problèmes de modification en place
            ...
            duree -= 1
            if duree <= 0:
                maj_capacites(chemin, capas_par, 1)
        ...
        for i in range(len(appels_dyn) - 1, -1, -1):
            ...
            duree -= 1
            if duree <= 0:
                maj_capacites(chemin, capas_dyn, 1)
                appels_dyn.pop(i)
            else:
...
""" # Lancer la simulation et afficher les résultats pour 1 certain nombre d'appels
nb_appels = 5000
...
# Liste du nombre d'appels
nb = [200*(i+1) for i in range(40)]

# Liste des quantités de rejet pour chaque itératon
liste_stat = []
liste_par_charge = []
liste_dynamique = []

# Listes des durées de calcul des routages
duree_stat = []
duree_par_charge = []
duree_dynamique = []
...
for i in nb:
    ...
    rejets_stat, rejets_par_charge, rejets_dyn, temps_stat, temps_par_charge, temps_dyn = simulation(i, liens_rezo, capacites, capacites, capacites)
    # Chance de rejet
